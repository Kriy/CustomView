...
public class FanView extends View {
    ...
    private void init(Context context, @Nullable AttributeSet attrs) {
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.FanView);
        mLineColor = ta.getColor(R.styleable.FanView_lineColor, Color.RED);
        mFirstFanRadiusRatio = ta.getFloat(R.styleable.FanView_fanRadiusRatio, FAN_RADIUS_RATIO);
        mFanSweepAngle = ta.getInt(R.styleable.FanView_fanSweepAngle, 80);
        mDuration = ta.getInt(R.styleable.FanView_fanDuration, 800);
        ta.recycle();
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mRadius = Math.min((w - Math.max(leftPadding, rightPadding)) / 2, (h - Math.max(topPadding, bottomPadding)) / 2) - ResourceUtil.dp2px(getContext(), 1);
    }
